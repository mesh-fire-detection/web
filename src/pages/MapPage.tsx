...
const DEPLOYMENT_LOG = [
  { date: '2026-08-21', event: 'Prospect East went dark. Battery protection cutout suspected — the cell was at 4% on its last packet.', kind: 'bad' as const },
  { date: '2026-07-05', event: 'Mailbox Spur and Mailbox Bowl installed. Neither has ever reported. Physical recovery scheduled.', kind: 'bad' as const },
  { date: '2026-06-14', event: 'Christmas Ridge and Christmas Saddle brought online. The 2.5 km hop from East Peak closed at 4.9 dB SNR.', kind: 'good' as const },
  { date: '2026-05-24', event: 'East Peak Base and its Vision node installed at 943 m. Best link budget in the network.', kind: 'good' as const },
  { date: '2026-05-02', event: 'Grand Prospect installed. Solar panel sited before leaf-out; it has been shaded ever since.', kind: 'warn' as const },
  { date: '2026-04-12', event: 'Branch head at Truck Road Gate, first Base node at Cedar Butte. Network exists.', kind: 'good' as const },
]
      ...
      <Section space="sm">
        <Stack gap={6}>
          <Box tone="transparent" border={false} padding={0} className="map-shell">
            <MapLegend />
            <LazyNetworkMap selectedId={selected?.id ?? null} onSelect={setSelected} height={560} />
            {selected ? <NodePanel node={selected} onClose={() => setSelected(null)} /> : null}
          </Box>

          {!HAS_BASEMAP ? (
            <Callout title="No basemap, on purpose" tone="fire" icon="map">
              The map renders node geometry on a blank canvas — no tile requests, no API key, no
              bill. That is the same instinct that produces a $70 node. Point{' '}
              <Text as="span" mono size="inherit" tone="fire">
                VITE_BASEMAP_STYLE
              </Text>{' '}
              at a Protomaps or MapTiler style and terrain appears underneath, unchanged otherwise.
            </Callout>
          ) : null}
          ...
          <Grid columns={5} minColumnWidth={150} gap={5}>
            ...
            />
            <Metric label="Longest hop" value={`${decimal(longestHop)} km`} size="lg" hint={`${totalHeight} m of mast total`} />
          </Grid>
        </Stack>
      </Section>
        ...
        <Stack gap={5}>
          ...
          <Stack gap={0}>
            {DEPLOYMENT_LOG.map((entry) => (
              <Row key={entry.date} gap={5} align="start" wrap={false} className="log__row">
                <Text as="span" size="xs" mono tone="faint" className="log__date">
                  {entry.date}
                </Text>
                <Text
                  size="sm"
                  tone={entry.kind === 'bad' ? 'dead' : entry.kind === 'warn' ? 'warn' : 'muted'}
                  measure={90}
                >
                  {entry.event}
                </Text>
