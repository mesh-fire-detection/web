...
function Column({ side }: { side: Side }) {
  ...
  return (
    <Box
      ...
      padding={6}
      radius="md"
      className={`cost__col cost__col--${side.kind}`}
      grow
    >
      <Stack gap={5}>
        ...
        <Stack gap={2}>
          <Heading level={3} size={ours ? '4xl' : '2xl'} tone={ours ? 'fire' : 'muted'} mono>
            {money(side.price)}
          </Heading>
...
export function CostComparison() {
  return (
    <Stack gap={5}>
      <Grid columns={2} gap={4} align="stretch" className="cost">
        <Column side={INCUMBENT} />
        <Column side={OURS} />
      </Grid>

      <Row gap={3} justify="center" className="cost__ratio">
        <Text as="div" size="sm" tone="muted" align="center" measure={90}>
          One camera site buys <Value tone="fire" size="sm">{RATIO}</Value> Base nodes. That ratio is
          the entire argument — everything else on this site exists to make it believable.
        </Text>
      </Row>
    </Stack>
  )
