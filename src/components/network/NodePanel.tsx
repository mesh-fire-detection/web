...
export function NodePanel({ node, onClose }: { node: MeshNode; onClose: () => void }) {
  ...
  return (
    <Box tone="surface-2" padding={4} radius="md" className="node-panel">
      <Stack gap={4}>
        <Row justify="between" align="start" gap={3} wrap={false}>
          <Stack gap={2} minWidth0>
            <StatusLabel kind={STATUS_KIND[node.status]} pulse={node.status === 'online'}>
              {node.status}
            </StatusLabel>
        ...
        <Grid columns={2} gap={4}>
          ...
          <Metric
            ...
            size="sm"
            tone={
              node.batteryPct === null ? 'muted' : node.batteryPct < 25 ? 'dead' : 'default'
            }
          />
          <Metric label="Last heard" value={sinceMinutes(node.lastHeartbeatMin)} size="sm" />
        ...
        {neighbours.length > 0 ? (
          <Stack gap={2}>
            ...
            {neighbours.map((link) => {
              ...
              return (
                <Row key={`${link.from}-${link.to}`} justify="between" gap={2} wrap={false}>
                  ...
                  <Row gap={2} wrap={false}>
                    ...
                    <Badge kind={quality === 'good' ? 'live' : quality === 'marginal' ? 'warn' : 'dead'} size="xs">
                      {link.snr} dB
                    </Badge>
                  </Row>
                </Row>
