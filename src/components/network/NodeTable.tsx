...
export function NodeTable({
  ...
  const columns: ReadonlyArray<Column<MeshNode>> = [
    {
      key: 'name',
      header: 'Node',
      render: (node) => (
        <BareButton className="node-row" onClick={() => onSelect(node)}>
          <Row gap={3} wrap={false}>
            <StatusDot kind={STATUS_KIND[node.status]} pulse={node.status === 'online'} />
    ...
    },
    {
      ...
      render: (node) =>
        node.batteryPct === null ? (
          ...
          <Value tone={node.batteryPct < 25 ? 'dead' : node.batteryPct < 50 ? 'warn' : 'default'}>
            {node.batteryPct}%
          </Value>
        ),
    },
    {
      ...
      width: '120px',
      render: (node) => (
        <Value tone={node.lastHeartbeatMin === null || node.lastHeartbeatMin > 60 ? 'dead' : 'muted'}>
