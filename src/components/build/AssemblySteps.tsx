    ...
    <Stack gap={4}>
      <Stack gap={0} className="steps">
        {ASSEMBLY.map((step, index) => (
          <Row key={step.title} gap={4} align="start" wrap={false} className="steps__row">
            <Text as="div" size="xs" mono weight={600} tone="fire" className="steps__num">
      ...
      <Box tone="surface-2" padding={4} radius="sm">
        <Text size="sm" tone="muted">
          About {total} minutes of hands-on work, plus print time and a 48-hour soak test. If you had
          to ask a question to get here, that is a bug — open an issue.
        </Text>
      </Box>
    </Stack>
  )
