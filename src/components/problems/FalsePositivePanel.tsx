...
import { FALSE_POSITIVE_STATE } from '@/data/problems'
    ...
    <Box tone="surface-2" padding={6} radius="md" id="false-positives">
      <Stack gap={6}>
        ...
        <Text size="md" tone="muted" measure={84}>
          Every cheap fire detection project dies on false positives — dust, fog, morning mist, a
          neighbour's burn pile. Publishing a measured rate is what gets a fire district to take a
          meeting. We do not have one yet, so here is the number we will publish and how we will get
          it.
        </Text>
        ...
        <Grid columns={3} minColumnWidth={150} gap={5}>
          ...
          <Metric label="Vision nodes reporting" value="2" size="md" hint="both in sample data" />
        </Grid>
        ...
        <Box tone="warn" border={false} accent="warn" padding={4} radius="sm">
          <Text size="sm" weight={600}>
            This network is not an emergency service. It does not contact 911, and no alert it
            produces is a substitute for calling one.
          </Text>
        </Box>
      </Stack>
    </Box>
