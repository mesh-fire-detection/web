...
export const TERRAINS: readonly TerrainSpec[] = [
    {
        id: 'open',
        label: 'Open / ridge to ridge',
        excessDbPerKm: 0,
        note: 'Clear line of sight, nothing in the first Fresnel zone.',
    },
    {
        id: 'light-forest',
        label: 'Light forest',
        excessDbPerKm: 2.5,
        note: 'Antennas above canopy, path clips treetops.',
    },
    {
        id: 'dense-forest',
        label: 'Dense forest',
        excessDbPerKm: 5,
        note: 'Path runs through standing timber. The Cascade default.',
    },
    {
        id: 'built-up',
        label: 'Built-up',
        excessDbPerKm: 8,
        note: 'Structures in the path. Rare for this network.',
    },
]
...
function requireSpec<T>(value: T | undefined, label: string): T {
    if (value === undefined) throw new Error(`${label} is missing from the spec tables.`)
    return value
}
...
function terrainSpec(id: Terrain): TerrainSpec {
    return requireSpec(
        TERRAINS.find((terrain) => terrain.id === id),
        `Terrain ${id}`
    )
}
...
function radioHorizonKm(txHeightM: number, rxHeightM: number): number {
    return 4.12 * (Math.sqrt(Math.max(txHeightM, 0)) + Math.sqrt(Math.max(rxHeightM, 0)))
}

/** First Fresnel zone radius at mid-path, metres. */
function fresnelRadiusM(distanceKm: number, frequencyMhz: number): number {
...
function receivedAt(distanceKm: number, input: LinkInput): number {
    const loss =
        fspl(distanceKm, input.frequencyMhz) + terrainSpec(input.terrain).excessDbPerKm * distanceKm
