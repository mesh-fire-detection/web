import type { CoverageContent } from '@core/content/types'

export const coverageContent = {
    title: 'Coverage calculator',
    eyebrow: 'Link budget',
    lede: 'Would this work where you live? Set two antenna heights and a distance and find out whether the hop closes — and how many nodes a corridor would take.',
    scope: {
        eyebrow: 'What this is and is not',
        title: 'First-order answer, not a viewshed',
        lede: 'This calculator has no elevation model behind it. It knows the earth is curved and it knows forest costs you signal. It does not know about the ridge between your two points.',
        modelsTitle: 'What it models',
        models: [
            'Free-space path loss at 915 MHz.',
            'A distance-proportional excess loss for vegetation and clutter.',
            'The 4/3-earth radio horizon from both antenna heights.',
            'First Fresnel zone radius, so you know how much clearance to hold.',
            'Receiver sensitivity for each Meshtastic preset.',
        ],
        omitsTitle: 'What it does not',
        omits: [
            'Terrain. There is no elevation data in this page.',
            'Diffraction over an obstruction, which can save a link this calculator calls dead.',
            'Ground reflection and multipath.',
            'Seasonal foliage, which is why the fade margin defaults to 10 dB.',
            'Interference from anything else on the band.',
        ],
        calloutTitle: 'Use this to rule sites out, not to commit to one',
        calloutBody:
            'If the calculator says a hop does not close, believe it — the real world only adds loss. If it says the hop closes, that is a hypothesis you confirm against terrain and then on site with two nodes and a walk.',
    },
    priorArt: {
        eyebrow: 'Prior art',
        title: 'Tools that do know about terrain',
        lede: 'There is no reason to rebuild any of this. When you have a real site, take the numbers from here into one of these.',
        openLabel: 'Open',
        tools: [
            {
                name: 'Meshtastic Site Planner',
                detail: 'Terrain-aware coverage for Meshtastic nodes, using real elevation data. Start here if you have a specific site.',
                href: 'https://site.meshtastic.org/',
            },
            {
                name: 'Splat!',
                detail: 'RF propagation over SRTM terrain, Longley-Rice ITM. The serious tool, and free.',
                href: 'https://www.qsl.net/kd2bd/splat.html',
            },
            {
                name: 'SRTM elevation data',
                detail: '30 m global elevation from NASA. What any terrain-aware viewshed is built on.',
                href: 'https://www.earthdata.nasa.gov/data/instruments/srtm',
            },
        ],
    },
    maths: {
        eyebrow: 'The maths',
        title: 'Everything this page computes',
        lede: 'No black box. These are the four equations behind every number on this page, and the implementation is a hundred lines you can read in the repository.',
        equations: [
            {
                term: 'Free-space path loss',
                value: 'FSPL(dB) = 20·log₁₀(d km) + 20·log₁₀(f MHz) + 32.44',
            },
            {
                term: 'Received power',
                value: 'P(dBm) = P_tx + G_tx + G_rx − L_cable − FSPL − (excess dB/km × d)',
            },
            {
                term: 'Radio horizon (4/3 earth)',
                value: 'd(km) = 4.12 · (√h₁ + √h₂), heights in metres',
            },
            {
                term: 'First Fresnel radius',
                value: 'r(m) = 17.31 · √(d₁·d₂ / (f GHz · d)), distances in km',
            },
            {
                term: 'Link closes when',
                value: 'Received power ≥ preset sensitivity + your fade margin, and d ≤ radio horizon',
            },
        ],
        footnote:
            'Sensitivity figures are typical SX1262 values for each spreading factor and bandwidth. Your radio will differ by a decibel or two, which is well inside the fade margin.',
    },
} as const satisfies CoverageContent
